	...
	public void onClick(View v)
	{
		switch (v.getId())
		{
			...
			case R.id.buttonc:
			{
				...
				DEBUG.log("DialActivity", "ret:" + ret);
				boolean fault = true;
				String[] rets = null;
				if (ret != null)
				{
					rets = ret.split(":");
					if (rets != null && rets.length == 2 && rets[0] != null && rets[1] != null)
					{
						mIp = rets[0];
						try
						{
							mPort = Integer.valueOf(rets[1]);
						}
						catch(Exception e)
						{
							mPort = 0;
						}
						if(Utils.isIpAddress(mIp) && mPort > 0)
						{
							fault = false;
						}
					}
				...
				if(fault)
				{
					Toast.makeText(DialActivity.this, "您输入的ip有问题！", Toast.LENGTH_SHORT).show();
				}
				else
				{
					Intent intent = new Intent();
					intent.setClass(DialActivity.this, CallActivity.class);					
					intent.putExtra(CallActivity.CONNECT_TYPE, true);
					intent.putExtra(CallActivity.CONNECT_IP, mIp);
					intent.putExtra(CallActivity.CONNECT_PORT, mPort);
